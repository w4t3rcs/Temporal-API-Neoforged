    ...
    protected void addDescriptions() {
        PARTICLE_SPRITES.forEach((particleType, particleSprite) -> {
            if (particleSprite.count() <= 1) {
                sprite(particleType.value(), IOHelper.createResourceLocation(particleSprite.id()));
            } else {
                spriteSet(particleType.value(), IOHelper.createResourceLocation(particleSprite.id()),
